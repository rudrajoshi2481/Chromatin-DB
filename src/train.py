...
from dashboard import TrainingDashboard, collect_fingerprints, collect_codebook_usage
...
def train_one_epoch(
    ...
    model.train()
    running = {k: 0.0 for k in ["total","recon","vq","boundary","compartment","boundary_f1","compartment_r"]}
    n_steps = 0
...
def validate(
    ...
    for batch in tqdm(loader, desc=f"Epoch {epoch:02d} [val]  ", leave=False):
        ...
        outputs = model(contact, assay_id)
        targets = {"contact": contact, "boundary": boundary, "compartment": compartment}
        _, metrics = total_loss(outputs, targets, epoch)
...
def train(
    ...
    train_loader, val_loader = build_dataloaders(
        cell_lines=list(active.keys()) if active else cell_lines,
    ...
    for epoch in range(epochs):
        ...
        print(
            ...
            f"bd_f1={val_metrics['boundary_f1']:.3f} "
            f"comp_r={val_metrics['compartment_r']:.3f} | "
